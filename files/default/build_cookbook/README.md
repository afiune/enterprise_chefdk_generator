...
Modify the `.kitchen.yml` if necessary to change the platforms or other configuration to run the verify phases. After making changes in the parent project, `cd` into this directory (`.delivery/enterprise_build_cookbook`), and run:
...
Each of the recipes in this enterprise_build_cookbook are run in the named phase during the Chef Delivery pipeline. The `unit`, `syntax`, and `lint` recipes are additionally run when using Test Kitchen for local testing as noted in the above section.
...
Next, increment the version in the metadata.rb. This should be in the _parent_, not in this, the enterprise_build_cookbook. If this is not done, the verify phase will fail.
...
From _this_ directory (`.delivery/enterprise_build_cookbook`, relative to the parent cookbook project), run

```
cd .delivery/enterprise_build_cookbook
kitchen converge
```
...
Now rerun kitchen from the enterprise_build_cookbook.
...
Make changes in the cookbook project as required for organizational goals and needs. Modify the `enterprise_build_cookbook` as necessary for the pipeline phases that the cookbook should go through.
